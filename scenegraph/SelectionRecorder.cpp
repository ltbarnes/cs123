...
void SelectionRecorder::enterSelectionMode(int x, int y, int numObjects)
{
    ...
    {
        cout << "error: enterSelectionMode() called twice without calling exitSelectionMode() " \
             << "in between" << endl;
        assert(false);
        return;
    }

    // Pre-multiply the projection matrix with a pick matrix, which expands the viewport to fill
    // the 1x1 square under the mouse pointer at (x, y).
    int viewport[4];
...
{
    // Replace the current name on top of the name stack with index. We only use a name stack of
    // depth one, so it's like a variable.
    glLoadName(index);
}
